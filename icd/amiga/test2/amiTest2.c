...
int main(int argc, char const* argv[])
{
    ...
    wcfg.title = AFX_STRING("Amiga Test 2");
    wcfg.dsys = dsys;
    ...
    while (1)
    {
        ...
        if (cursor < totalFrames)
        {
            ...
            if (!AmxLockSinkBuffer(sink, AFX_TIMEOUT_IGNORED, NIL, writable, &room))
            {
                ...
                AmxPrepareMixCommands(mix, FALSE, amxCmdFlag_ONCE);
