...
_ZGL afxError _ZglDsysCtorCb(afxDrawSystem dsys, void** args, afxUnit invokeNo)
{
    ...
    dsys->robustCtx = !!cfg->cfg.features.robustness;
    ...
    else
    {
        ...
        else
        {
            ...
            for (afxUnit i = 0; i < cfg->cfg.exuCnt; i++)
            {
                //bridgeCfgs[i].dctxClsCfg = &dctxClsCfg;
                //bridgeCfgs[i].dctxClsCfg = &dqueClsCfg;
            }

            if (_AVX_CLASS_CONFIG_DSYS.ctor(dsys, (void*[]) { icd, (void*)&cfg2 }, 0))
            {
                AfxThrowError();
            }
            else
