...
_ASX afxError _AsxSpuRollContext(asxSpu* spu, arxContext ctx)
{
    afxError err = { 0 };
    ...
    arxSimBridge sexu = spu->sexu;
    asxCmdList const*cmdVmt = sexu->cmdVmt;
    afxCmdId lastId = 0; // DBG

    asxCmd *cmdHdr;
    AFX_ITERATE_CHAIN_B2F(cmdHdr, hdr.script, &ctx->commands)
    {
...
_ASX afxError _AsxCtxResetCb(arxContext ctx, afxBool freeMem, afxBool permanent)
{
    afxError err = { 0 };
    AFX_ASSERT(ctx->state != asxContextState_PENDING);
...
_ASX afxError _AsxCtxEndCb(arxContext ctx)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_CTX, 1, &ctx);
    return err;
