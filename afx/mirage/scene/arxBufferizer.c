...
_ARX afxError AfxAcquireBufferizer(arxScenario scio, arxBufferizerInfo const* info, arxBufferizer bufferizer[])
{
    afxError err = { 0 };
    ...
    afxClass* cls = &(afxClass) { 0 }; //_ArxScioGetVbufClass(scio);
    AFX_ASSERT_CLASS(cls, afxFcc_VBUF);

    if (AfxAcquireObjects(cls, 1, (afxObject*)bufferizer, (void const*[]) { scio, info }))
        AfxThrowError();
