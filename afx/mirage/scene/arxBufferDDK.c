...
_ARX afxError _ArxSmemCtorCb(_arxMemory* smem, void** args, afxUnit invokeNo)
{
    ...
    //AfxMakeChain(&smem->buffers, smem);
...
_ARX afxClassConfig const _ARX_SMEM_CLASS_CONFIG =
{
    .fcc = afxFcc_SMEM,
    .name = "SynergicMemory",
...
#if 0
_ARX afxError AfxReallocateBuffer(arxBuffer sbuf, afxUnit unit, afxSize offset)
{
    afxError err;
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &sbuf);

    _acxMemory* sbs = AfxGetLinker(&sbuf->mem);
