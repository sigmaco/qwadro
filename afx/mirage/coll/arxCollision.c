...
_ARX void precompute_triangle_edges(afxUnit triCnt, afxUnit const indices[], afxV3d const vertices[], asxTriangle tris[])
{
...
_ARX void transform_triangle_edges_and_normal(afxM4d const m, asxTriangle const* in, asxTriangle* out)
{
...
_ARX afxBool intersect_triangle(asxTriangle const* triA, asxTriangle const* triB)
{
...
_ARX afxBool AfxTestShapeCollision2(asxShape const s1, afxM4d const m1, asxShape const s2, afxM4d const m2)
{
...
_ARX afxBool AfxTestShapeCollision(asxShape const s1, asxShape const s2)
{
    // Edge-to-edge intersection test using Moller–Trumbore algorithm
...
_ARX afxError _AsxShapDtorCb(asxShape shap)
{
    afxError err = { 0 };
...
_ARX afxError _AsxShapCtorCb(asxShape shap, void** args, afxUnit invokeNo)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SHAP, 1, &shap);
...
_ARX afxClassConfig const _ASX_SHAP_CLASS_CONFIG =
{
    .fcc = afxFcc_SHAP,
    .name = "Shape",
...
_ARX afxError AsxAcquireShapes(arxSimulation sim, afxUnit cnt, asxShapeInfo const info[], asxShape shapes[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SIM, 1, &sim);
...
_ARX afxError ArxBuildMeshShape(arxSimulation sim, arxMesh msh, asxShape* shape)
{
    afxError err = { 0 };
