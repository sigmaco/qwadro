...
_AFX afxError _AfxDevProcCb(afxDevice dev, afxThread thr, void* extra)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_DEV, 1, &dev);
...
_AFX afxResult _AfxDevIoCtlCb(afxDevice dev, afxUnit code, va_list va)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_DEV, 1, &dev);
    return 0;
        ...
        //else
        {
            ...
            //else
            {
                ...
                if (!err)
                {
                    ...
                    if (!err)
                    {
                        dev->procCb = info->procCb ? info->procCb : _AfxDevProcCb;
                        dev->ioctlCb = info->ioctl ? info->ioctl : _AfxDevIoCtlCb;
