...
_AFX afxClass* _AfxGetSysMgr(void)
{
    ...
    static afxClassConfig const sysClsCfg =
    {
        ...
        .fixedSiz = 0,
        .ctor = (void*)_AfxSysCtorCb,
        .dtor = (void*)_AfxSysDtorCb
    };
...
_AFX afxClass const* _AfxSysGetXssClass(afxSystem sys)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SYS, 1, &sys);
    afxClass const* cls = &sys->xssCls;
    AFX_ASSERT_CLASS(cls, afxFcc_XSS);
    return cls;
}

_AFX afxError AfxGetSystemDirectory(afxUri *dst)
{
    afxError err = { 0 };
    afxSystem sys;
    AfxGetSystem(&sys);
    ...
    return AfxCopyUri(dst, qwd);
}

_AFX afxUri const* _AfxGetSystemDirectory(afxUri *dst)
{
    afxError err = { 0 };
    afxSystem sys;
    AfxGetSystem(&sys);
...
_AFX afxUnit AfxGetSystemDirectoryString(afxString *dst)
{
    afxError err = { 0 };
    afxSystem sys;
    AfxGetSystem(&sys);
    ...
    return AfxCopyString(dst, 0, qwd, 0);
}

_AFX afxString const* _AfxGetSystemDirectoryString(afxString *dst)
{
    afxError err = { 0 };
    afxSystem sys;
    AfxGetSystem(&sys);
...
_AFX afxError AfxGetPwd(afxUri *dst)
{
    afxError err = { 0 };
    afxSystem sys;
    AfxGetSystem(&sys);
    ...
    return AfxCopyUri(dst, pwd);
}

_AFX afxUri const* _AfxGetPwd(afxUri *dst)
{
    afxError err = { 0 };
    afxSystem sys;
    AfxGetSystem(&sys);
...
_AFX afxUnit AfxGetPwdString(afxString *dst)
{
    afxError err = { 0 };
    afxSystem sys;
    AfxGetSystem(&sys);
    ...
    return AfxCopyString(dst, 0, pwd, 0);
}

_AFX afxString const* _AfxGetPwdString(afxString *dst)
{
    afxError err = { 0 };
    afxSystem sys;
    AfxGetSystem(&sys);
...
_AFX afxError _AfxDoSubsystemShutdown(afxSystem sys)
{
    ...
    if (sys->acx.ssys)
    {
        sys->acx.ioctl(sys, sys->acx.e2soundDll, 4, NIL);
    }
#endif
    if (sys->e2mixDll)
    {
    ...
    //AfxZero(&sys->acx, sizeof(sys->acx));
...
_AFX afxBool _AfxLoadAndAttachIcd(void* udd, afxUnit diskId, afxUnit endpointIdx, afxUri const* path, afxUri const* osPath)
{
    ...
    afxString8 subsys;
    AfxMakeString8(&subsys, NIL);
    if (!AfxGetInitializationString(&ini, &AFX_STRING("Qwadro.Icd"), &AFX_STRING("Subsystem"), &subsys.s))
    {
        AfxThrowError();
    ...
    {
        afxBool proceed = FALSE;

        if (0 == AfxCompareString(&subsys.s, 0, platform->start, 0, FALSE))
        {
            proceed = TRUE;
        }
        ...
        if (proceed)
        {
            ...
            afxModule icd;
            if (AfxAcquireModule(&dllFile.uri, NIL, &icd))
                AfxThrowError();
        }
    }
...
_AFX afxError _AfxScanIcdManifests(afxSystem sys, afxString const* platform, afxManifest const* sysIni)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SYS, 1, &sys);
    ...
    AfxFindFiles(&fileMask, afxFileFlag_R, (void*)_AfxLoadAndAttachIcd, (void*)platform);
#if 0
    afxFile file;
    ...
    if (!AfxOpen(&fileMask, afxFileFlag_R, &file))
    {
        do
        {
            ...
            if (AfxAcquireModule(&dllFile.uri, NIL, &icd)) AfxThrowError();
            else
            {
...
_AFX afxError AfxBootstrapSystem(afxSystemConfig const *config)
{
    ...
    if (!err)
    {
        ...
        if (AfxAcquireModule(&uri, AFX_BITMASK(8), &e2drawDll)) AfxThrowError();
        else
        {
    ...
    if (!err)
    {
        ...
        if (AfxAcquireModule(&uri, AFX_BITMASK(8), &e2mixDll)) AfxThrowError();
        else
        {
    ...
    dpyClsCfg.ctor = NIL;
    dpyClsCfg.dtor = NIL;
    AfxMountClass(&sys->dpyCls, (afxClass*)_AfxSysGetDevClass(sys), &sys->classes, &dpyClsCfg);
    ...
    if (!err)
    {
        ...
        if (AfxAcquireModule(&uri, AFX_BITMASK(8), &e2mmuxDll)) AfxThrowError();
        else
        {
    ...
    _AfxScanIcdManifests(sys, &AFX_STRING("AFX"), &ini);
    _AfxScanIcdManifests(sys, &AFX_STRING("AUX"), &ini);
    _AfxScanIcdManifests(sys, &AFX_STRING("AVX"), &ini);
    _AfxScanIcdManifests(sys, &AFX_STRING("AMX"), &ini);
...
_AFX void AfxAbolishSystem(afxInt exitCode)
{
    afxError err = { 0 };
