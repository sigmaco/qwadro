...
_AUX afxError _AuxFntCtorCb(afxFont fnt, void** args, afxUnit invokeNo)
{
    ...
    if (cfg->dsys && cfg->isDevFont)
    {
        ...
        vlay.binCnt = 1;
        vlay.bins[0] = AVX_VERTEX_STREAM(0, 0, 1);
        vlay.attrCnt = 3;
        vlay.attrs[0] = AVX_VERTEX_ATTR(0, 0,  0, avxFormat_RG32f);
        vlay.attrs[1] = AVX_VERTEX_ATTR(1, 0,  8, avxFormat_RG32f);
        vlay.attrs[2] = AVX_VERTEX_ATTR(2, 0, 16, avxFormat_RGBA8un);
