...
_ACX afxClass const* _AcxSexuGetSqueClass(afxWarpBridge sexu)
{
    afxError err = { 0 };
...
_ACX afxWarpSystem AcxGetBridgedWarpSystem(afxWarpBridge sexu, afxUnit* bridgeId)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
...
_ACX afxWarpDevice AcxGetBridgedWarpDevice(afxWarpBridge sexu, afxUnit* sdevId)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
    afxWarpDevice sdev = sexu->sdev;
    AFX_ASSERT_OBJECTS(afxFcc_SDEV, 1, &sdev);
    if (sdevId) *sdevId = AfxGetObjectId(sdev);
    return sdev;
}

_ACX afxUnit AcxGetWarpQueues(afxWarpBridge sexu, afxUnit baseQueIdx, afxUnit cnt, afxWarpQueue queues[])
{
    afxError err = { 0 };
    // sexu must be a valid afxWarpBridge handle.
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
    // queues must be a valid pointer to the afxWarpQueue handles.
    AFX_ASSERT(queues);
...
_ACX afxError AcxWaitForEmptyWarpQueue(afxWarpBridge sexu, afxUnit queIdx, afxUnit64 timeout)
{
    afxError err = { 0 };
    // sexu must be a valid afxWarpBridge handle.
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
    ...
    afxWarpQueue sque;
    if (!AcxGetWarpQueues(sexu, queIdx, 1, &sque))
    {
        AfxThrowError();
        return err;
    }
...
_ACX afxError AcxWaitForIdleWarpBridge(afxWarpBridge sexu, afxUnit64 timeout)
{
    afxError err = { 0 };
    // sexu must be a valid afxWarpBridge handle.
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
    ...
    for (afxUnit i = 0; i < queCnt; i++)
        AcxWaitForEmptyWarpQueue(sexu, i, timeout);

    return err;
}

_ACX afxError _AcxSexuTransferWarpMemory(afxWarpBridge sexu, acxTransference* ctrl, afxUnit opCnt, void const* ops)
{
    afxError err = { 0 };
    // @sexu must be a valid afxWarpBridge handle.
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
...
_ACX afxError _AcxSexuRemapBuffers(afxWarpBridge sexu, afxBool unmap, afxUnit cnt, _acxBufferRemapping const maps[])
{
    ...
    afxWarpQueue sque;
    afxUnit queIdx = 0;
    while (AcxGetWarpQueues(sexu, queIdx++, 1, &sque))
    {
        AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
    ...
    if (queued)
    {
        ...
        //if (AcxWaitForEmptyWarpQueue(sque, AFX_TIMEOUT_INFINITE))
        if (AcxWaitForEmptyWarpQueue(sexu, queIdx - 1, AFX_TIMEOUT_INFINITE))
            AfxThrowError();
    }
    return err;
}

_ACX afxError _AcxSexuCohereMappedBuffers(afxWarpBridge sexu, afxBool discard, afxUnit cnt, acxBufferedMap const maps[])
{
    afxError err = { 0 };
    // @sexu must be a valid afxWarpBridge handle.
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
...
_ACX afxError _AcxSexuDtorCb(afxWarpBridge sexu)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);

    afxWarpSystem ssys = AcxGetBridgedWarpSystem(sexu, NIL);
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);

    AcxWaitForWarpSystem(ssys, AFX_TIMEOUT_INFINITE);
    AcxWaitForWarpSystem(ssys, AFX_TIMEOUT_INFINITE); // yes, two times.
...
_ACX afxError _AcxSexuCtorCb(afxWarpBridge sexu, void** args, afxUnit invokeNo)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);

    afxWarpSystem ssys = AFX_CAST(afxWarpSystem, args[0]);
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
...
_ACX afxClassConfig const _ACX_SEXU_CLASS_CONFIG =
{
    .fcc = afxFcc_SEXU,
    .name = "WarpBridge",
    .desc = "Warp Device Execution Bridge",
    .fixedSiz = sizeof(AFX_OBJECT(afxWarpBridge)),
...
_ACX afxError _AcxSsysBridgeDevices(afxWarpSystem ssys, afxUnit cnt, _acxSexuAcq const configs[], afxWarpBridge bridges[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    AFX_ASSERT(configs);
