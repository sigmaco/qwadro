...
_ACX afxWarpSystem AcxGetBufferHost(acxBuffer buf)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    afxWarpSystem ssys = AfxGetHost(buf);
...
_ACX afxError AcxAcquireBuffers(afxWarpSystem ssys, afxUnit cnt, acxBufferInfo const infos[], acxBuffer buffers[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    AFX_ASSERT(buffers);
