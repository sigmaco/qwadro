...
_ACX afxError _AcxSqueUnlockIoReqChain(afxWarpQueue sque)
{
    afxError err = { 0 };
...
_ACX afxError _AcxSqueLockIoReqChain(afxWarpQueue sque, afxUnit64 timeout)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
...
_ACX afxError _AcxSquePopBlob(afxWarpQueue sque, void* blob, afxUnit siz)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
...
_ACX void* _AcxSquePushBlob(afxWarpQueue sque, afxUnit siz)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
...
_ACX afxError _AcxSquePopIoReqPacket(afxWarpQueue sque, _acxIoReqPacket* iorp)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
    AFX_ASSERT(iorp);
...
_ACX afxError _AcxSquePushIoReqPacket(afxWarpQueue sque, afxUnit id, afxUnit siz, afxCmdId* cmdId, _acxIoReqPacket** pIorp)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
...
_ACX afxWarpDevice AcxGetWarpQueueDevice(afxWarpQueue sque)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
    afxWarpBridge sexu = AfxGetHost(sque);
...
_ACX afxWarpSystem AcxGetWarpQueueSystem(afxWarpQueue sque)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
    afxWarpSystem ssys = sque->ssys;
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    return ssys;
}

_ACX afxError _AcxSqueSubmitCallback(afxWarpQueue sque, afxError(*f)(void*, void*), void* udd)
{
    afxError err = { 0 };
...
_ACX afxError _AcxSqueSubmitWarpCommands(afxWarpQueue sque, afxUnit cnt, acxSubmission subms[])
{
    afxError err = { 0 };
    // sque must be a valid afxWarpQueue handle.
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
    ...
    */
    ...
    else
    {
        ...
        for (afxUnit i = 0; i < cnt; i++)
        {
            ...
            afxWarpContext sctx = subms[i].sctx;
...
_ACX afxError _AcxSqueTransferResources(afxWarpQueue sque, acxTransference const* ctrl, afxUnit opCnt, void const* ops)
{
    afxError err = { 0 };
    // sque must be a valid afxWarpQueue handle.
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
...
_ACX afxError _AcxSqueCohereMappedBuffers(afxWarpQueue sque, afxUnit flushCnt, acxBufferedMap const flushes[], afxUnit fetchCnt, acxBufferedMap const fetches[])
{
    afxError err = { 0 };
    // sque must be a valid afxWarpQueue handle.
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
...
_ACX afxError _AcxSqueDtorCb(afxWarpQueue sque)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);
...
_ACX afxError _AcxSqueCtorCb(afxWarpQueue sque, void** args, afxUnit invokeNo)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SQUE, 1, &sque);

    afxWarpBridge sexu = args[0];
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
    ...
    sque->ssys = AcxGetBridgedWarpSystem(sexu, NIL);
...
_ACX afxClassConfig const _ACX_SQUE_CLASS_CONFIG =
{
    .fcc = afxFcc_SQUE,
    .name = "WarpQueue",
    .desc = "Warp Device Queue",
    .fixedSiz = sizeof(AFX_OBJECT(afxWarpQueue)),
