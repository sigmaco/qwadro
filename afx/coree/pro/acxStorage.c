...
_ACX afxError _AcxSsysRemapBuffersCb_SW(afxWarpSystem ssys, afxBool unmap, afxUnit cnt, _acxBufferRemapping const maps[])
{
    ...
    if (dedIoExuMask)
    {
        exuCnt = AcxChooseWarpBridges(ssys, AFX_INVALID_INDEX, acxAptitude_DMA, dedIoExuMask, 0, 0, NIL);
        AFX_ASSERT(exuCnt);
        exuIdx = 0;
        while (AcxChooseWarpBridges(ssys, AFX_INVALID_INDEX, acxAptitude_DMA, dedIoExuMask, exuIdx++, 1, &mexu))
        {
    ...
    if (!queued)
    {
        exuCnt = AcxChooseWarpBridges(ssys, AFX_INVALID_INDEX, acxAptitude_DMA, ioExuMask, 0, 0, NIL);
        AFX_ASSERT(exuCnt);
        exuIdx = 0;
        while (AcxChooseWarpBridges(ssys, AFX_INVALID_INDEX, acxAptitude_DMA, ioExuMask, exuIdx++, 1, &mexu))
        {
            queErr = _AcxSexuRemapBuffers(mexu, unmap, cnt, maps);
            err = queErr;
