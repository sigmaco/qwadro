...
_ACX afxError _AcxSctxExhaustCb(afxWarpContext sctx, afxBool freeMem)
{
    afxError err = { 0 };
...
#endif
            if (AcxPrepareWarpCommands(aux, freeMem, NIL))
                AfxThrowError();
}
    ...
    if (AcxPrepareWarpCommands(sctx, freeMem, NIL))
        AfxThrowError();
#if 0
...
_ACX afxError _AcxSctxPrepareCb(afxWarpContext sctx, afxBool purge, acxCmdFlags flags)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCTX, 1, &sctx);
...
_ACX afxError _AcxSctxCompileCb(afxWarpContext sctx)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCTX, 1, &sctx);
...
_ACX afxError _AcxSctxRecycleCb(afxWarpContext sctx, afxBool freeRes)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCTX, 1, &sctx);
    ...
    AcxPrepareWarpCommands(sctx, freeRes, NIL);

    afxWarpContext pool = AcxGetCommandPool(sctx);
