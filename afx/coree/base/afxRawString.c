...
//#define USE_SSE_STRLEN 1
//#define USE_SSE_STRCMP 1
//#define USE_SSE_STRCPY 1
//#define USE_SSE_STRNCMP 1
//#define USE_SSE_STRNICMP 1
//#define USE_SSE_STRNCPY 1
//#define USE_SSE_STRICMP 1
//#define USE_SSE_STRNCAT 1
//#define USE_SSE_STRCAT 1
...
_AFXINL afxSize utf8codepointsize(afxInt32 ch)
{
...
_AFXINL afxResult AfxStrncmpSSE(afxChar const* s1, afxChar const* s2, afxUnit len)
{
    /*
        strncmp compares the first n characters of two strings. With SSE 4.2, 
        we can compare multiple characters at once using the PCMPEQB and PMOVMSKB instructions.
    */
    ...
    while (i < len)
    {
        ...
        __m128i chunk1 = _mm_loadu_si128((__m128i*)(s1 + i));
        __m128i chunk2 = _mm_loadu_si128((__m128i*)(s2 + i));
        ...
        if (mask != 0xFFFF) {
            // Find the first mismatch and return the difference
            return (s1[i + __builtin_ctz(~mask)] - s2[i + __builtin_ctz(~mask)]);
        }

        i += 16;

        if (s1[i] == '\0' || s2[i] == '\0') {
            break;  // Handle null termination early
        }
    }
...
{
#ifdef USE_SSE_STRNCMP
    return AfxStrncmpSSE(str, str2, len);
#else
...
_AFXINL afxResult AfxStrcmpSSE(afxChar const* s1, afxChar const* s2)
{
    /*
        For comparing two strings, we can leverage the PCMPEQB instruction and the PMOVMSKB to compare 16 bytes at a time. 
        If a mismatch occurs, we can immediately return the result.
    */
    ...
    {
        // Load 16 bytes from each string
        chunk1 = _mm_loadu_si128((__m128i*)(s1 + i));
        chunk2 = _mm_loadu_si128((__m128i*)(s2 + i));
        ...
        if (mask != 0xFFFF)
        {
            // If not all bytes are equal, find the first mismatch
            return (s1[i + __builtin_ctz(~mask)] - s2[i + __builtin_ctz(~mask)]);
}

        // If we didn't find a mismatch, continue checking the next 16 bytes
        i += 16;
    }
    return 0;  // Strings are equal
}

_AFXINL afxResult AfxStrcmp(afxChar const* s1, afxChar const* s2)
{
#ifdef USE_SSE_STRCMP
    return AfxStrcmpSSE(s1, s2);
#else
    return strcmp(s1, s2);
#endif
#if 0
...
_AFXINL afxChar* AfxStrcpySSE(afxChar* dst, afxChar const* src)
{
    /*
        For operations like copying or concatenating strings, we can use the MOVDQU or MOVUPS instructions to load and store 16 bytes at a time.
    */
    ...
    while (1)
    {
        // Load 16 bytes from the source string
        chunk = _mm_loadu_si128((__m128i*)&src[i]);

        // Store the 16 bytes into the destination
        _mm_storeu_si128((__m128i*)&dst[i], chunk);

        // If we've hit the null terminator, stop
        if (((char*)&chunk)[15] == '\0') break;
...
{
#ifdef USE_SSE_STRCPY
    return AfxStrcpySSE(dst, src);
#else
...
_AFXINL afxSize AfxStrlenSSE(afxChar const *str)
{
    /*
        We can use the PCMPEQB instruction to compare chunks of the string against the null terminator ('\0'). 
        For example, we can load 16 bytes at a time and check if any byte is zero. If all bytes are non-zero, 
        continue scanning; otherwise, stop once we find a null terminator.
    */
    ...
    while (1)
    {
        // Load 16 bytes at a time from the string
        chunk = _mm_loadu_si128((__m128i*)(str + i));

        // Compare the chunk to zero
        ...
        if (mask)
        {
            // If any byte is zero, find the position of the null byte
            return i + /*__builtin_ctz*/AfxCountTrailingZeros(mask);  // Using GCC's builtin to count trailing zeros
        }
...
_AFXINL afxSize AfxStrlen(afxChar const* str)
{
#ifdef USE_SSE_STRLEN
    return AfxStrlenSSE(str);
#else
    return strlen(str);
#endif
#if 0
...
_AFXINL afxChar* AfxStrcatSSE(afxChar* dst, afxChar const* src)
{
    /*
        strcat appends a string to the end of another. 
        The general approach for strcat is similar to strncat, 
        but we need to first find the null terminator of the destination string and then concatenate the source string.
    */

    afxSize dst_len = strlen(dst);
    afxSize i = dst_len;
    ...
    while (*src) {
        ...
        // Store the 16 bytes to the destination
        _mm_storeu_si128((__m128i*)&dst[i], chunk);

        i += 16;
}

    dst[dst_len + strlen(src)] = '\0';  // Null terminate the string
    return dst;
}

_AFXINL afxChar* AfxStrcat(afxChar* dst, afxChar const* src)
{
#ifdef USE_SSE_STRCAT
    return AfxStrcatSSE(dst, src);
#else
...
_AFXINL afxChar* AfxStrncatSSE(afxChar* dst, afxChar const* src, afxUnit len)
{
    /*
        strncat appends up to n characters from the source string to the destination string. 
        After finding the null byte in the destination string, we can concatenate efficiently using SSE.
    */

    afxSize dst_len = strlen(dst);
    afxSize i = dst_len;

    while (i < dst_len + len && *src)
    {
        // Load 16 bytes from the source string
        __m128i chunk = _mm_loadu_si128((__m128i*)&src[i]);
    ...
    dst[dst_len + len] = '\0';  // Null terminate the string
    return dst;
}

_AFXINL afxChar* AfxStrncat(afxChar* dst, afxChar const* src, afxUnit len)
{
#ifdef USE_SSE_STRNCAT
    return AfxStrncatSSE(dst, src, len);
#else
...
_AFXINL afxResult AfxStricmpSSE(afxChar const* s1, afxChar const* s2)
{
    /*
        stricmp compares two strings case-insensitively. 
        This is similar to strnicmp but compares the entire string rather than just the first n characters. 
        We can use sse_strnicmp to handle this efficiently.
    */
    ...
    while (1)
    {
        ...
        // Convert both chunks to lowercase
        chunk1 = _mm_andnot_si128(_mm_set1_epi8(0x20), chunk1);
        chunk2 = _mm_andnot_si128(_mm_set1_epi8(0x20), chunk2);
...
{
#ifdef USE_SSE_STRICMP
    return AfxStricmpSSE(str, str2);
#else
...
_AFXINL afxChar* AfxStrncpySSE(afxChar* dst, afxChar const* src, afxSize len)
{
    /*
        strncpy copies up to n characters from one string to another. 
        If the source string is shorter than n, it fills the rest of the destination with null bytes.
    */
    ...
    while (i < len)
    {
        ...
        if (src[i + 15] == '\0') break;  // Stop if null byte is encountered

        i += 16;
    }

    // If n is greater than the length of the source string, pad the rest with '\0'
    for (; i < len; i++)
    {
        dst[i] = '\0';
    }
...
{
#ifdef USE_SSE_STRNCPY
    return AfxStrncpySSE(dst, src, len);
#else
...
_AFXINL afxResult AfxStrnicmpSSE(afxChar const* s1, afxChar const* s2, afxSize len)
{
    /*
        strnicmp compares the first n characters of two strings case-insensitively. 
        We can convert the strings to lowercase or uppercase on the fly and then compare them.
    */
...
{
#ifdef USE_SSE_STRNICMP
    return AfxStrnicmpSSE(str, str2, len);
#else
...
_AFXINL afxChar* AfxStrchrSSE(afxChar const* str, afxInt ch)
{
    /*
        strchr searches for the first occurrence of a character in a string. 
        We'll use SSE to scan chunks of the string at once and find the first match.
    */

    __m128i target = _mm_set1_epi8((char)ch);  // Load the target character into an SSE register
    afxSize i = 0;
    __m128i chunk;
    ...
    while (1) {
        ...
        __m128i result = _mm_cmpeq_epi8(chunk, target);

        // Create a mask from the result (1 where a match occurs)
        int mask = _mm_movemask_epi8(result);

        if (mask)
        {
            // If a match is found, return the corresponding character pointer
            afxSize pos = i + __builtin_ctz(mask);
            return (char*)&str[pos];
        }

        // If we've reached a null terminator, return NULL (no match found)
        if (str[i] == '\0')
        {
            break;
        }
    ...
    return NULL;  // Return NULL if character was not found
}
...
_AFXINL afxChar* AfxStrrchrSSE(afxChar const* str, afxInt ch)
{
    /*
        strrchr searches for the last occurrence of a character in a string. 
        To efficiently find the last occurrence using SSE, we can scan the string from the end toward the beginning 
        (similar to the traditional approach but in parallel).
    */

    __m128i target = _mm_set1_epi8((char)ch);  // Load the target character into an SSE register
    afxSize len = strlen(str);
    afxSize i = len - 1;  // Start at the end of the string
    __m128i chunk;
    ...
    while (i > 0)
    {
        ...
        i -= 16;  // Move backwards by 16 characters
    }

    // Handle the first 16 characters separately if the string isn't a multiple of 16
