...
extern afxClassConfig const _AFX_CLASS_CONFIG_ARC;
...
_AFX afxError _DismountStorageUnit(afxStorage fsys, _afxFsysUnit* fsto)
{
    afxError err = { 0 };
    AFX_ASSERT(fsto);
...
_AFX afxError _MountStorageUnit(afxStorage fsys, afxUri const* endpoint, afxFileFlags fileFlags)
{
    ...
    if (AfxIsUriPathRelative(endpoint))
    {
        ...
        if (AfxExcerptUriExtension(&archiveNameExtUri, endpoint, FALSE))
        {
            ...
            if (AfxOpenArchive(endpoint, fileFlags, &arc)) AfxThrowError();
            else
            {
                afxUri path;
        ...
        else
        {
            ...
            {
                afxUri const *rootUri = _AfxGetSystemDirectory(NIL);
                afxString const *rootUriString = _AfxGetSystemDirectoryString(NIL);
...
_AFX afxError _AfxFsysCtorCb(afxStorage fsys, void** args, afxUnit invokeNo)
{
    ...
    else
    {
        ...
        AfxMountClass(&fsys->archCls, NIL, &fsys->classes, &_AFX_CLASS_CONFIG_ARC); // require iob
...
_AFX afxUnit AfxEnumerateStorages(afxUnit first, afxUnit cnt, afxStorage storages[])
{
    afxError err = { 0 };
    AFX_ASSERT(cnt);
    AFX_ASSERT(storages);
    ...
    afxClass* cls = (afxClass*)_AfxSysGetFsysClass(sys);
    AFX_ASSERT_CLASS(cls, afxFcc_FSYS);
    return AfxEnumerateObjects(cls, first, cnt, (afxObject*)storages);
}
