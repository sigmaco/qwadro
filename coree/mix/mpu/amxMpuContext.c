...
#if 0
_AMXINL void AfxQueryMixState(afxMixContext mctx, arxPulleyState* state)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_MCTX, 1, &mctx);
    ...
    AFX_ASSERT(state);
    state->active = 1;// (mctx->motor.flags & arxPulleyFlag_ACTIVE) == arxPulleyFlag_ACTIVE;
    state->speed = speed;
...
#if 0
    if (mctx->motor.flags & arxPulleyFlag_FORCE_CLAMPLED_LOOPS)
    {
