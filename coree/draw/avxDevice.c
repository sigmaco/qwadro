...
_AVX void* _AvxDdevGetIdd(afxDrawDevice ddev)
{
    afxError err = { 0 };
    // @ddev must be a valid afxDrawDevice handle.
    AFX_ASSERT_OBJECTS(afxFcc_DDEV, 1, &ddev);
...
_AVX void AvxQueryDrawCapabilities(afxDrawDevice ddev, avxDeviceInfo* caps)
{
    afxError err = { 0 };
    // @ddev must be a valid afxDrawDevice handle.
    AFX_ASSERT_OBJECTS(afxFcc_DDEV, 1, &ddev);
...
_AVX afxBool AvxIsDrawDeviceAcceptable(afxDrawDevice ddev, avxFeatures const* features, avxLimits const* limits)
{
    ...
    if (features)
    {
        if ((features->robustness && !ddev->features.robustness) ||
            ...
            (features->tesselation && !ddev->features.tesselation) ||
            ...
            (features->mdi && !ddev->features.mdi) ||
            (features->baseInst && !ddev->features.baseInst) ||
            ...
            (features->viewports && !ddev->features.viewports) ||
            ...
            (features->clipDist && !ddev->features.clipDist) ||
            (features->cullDist && !ddev->features.cullDist) ||
...
_AVX afxUnit AvxEnumerateDrawDevices(afxUnit icd, afxUnit first, afxUnit cnt, afxDrawDevice devices[])
{
    ...
    AFX_ASSERT(cnt);
    AFX_ASSERT(devices);
    afxUnit rslt = AfxEnumerateObjects(cls, first, cnt, (afxObject*)devices);
...
_AVX afxUnit AvxEvokeDrawDevices(afxUnit icd, afxUnit first, void* udd, afxBool(*f)(void*, afxDrawDevice), afxUnit cnt, afxDrawDevice devices[])
{
    ...
    AFX_ASSERT(cnt);
    AFX_ASSERT(devices);
    afxUnit rslt = AfxEvokeObjects(cls, (void*)f, udd, first, cnt, (afxObject*)devices);
...
_AVX afxUnit AvxChooseDrawDevices(afxUnit icd, avxDeviceInfo const* caps, avxFeatures const* features, avxLimits const* limits, afxUnit maxCnt, afxUnit ddevId[])
{
    ...
    for (afxUnit i = 0; TRUE; i++)
    {
        ...
        if (caps)
        {
            ...
            if (ddev->nonRhcs != caps->nonRhcs)
                continue;

            if (caps->clipSpaceDepth && (!(ddev->clipSpaceDepth & caps->clipSpaceDepth)))
            {
                continue;
            }
        }
...
_AVX afxError _AvxIcdRegisterDevices(afxModule icd, afxUnit cnt, _avxDdevReg const infos[], afxDrawDevice devices[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_MDLE, 1, &icd);
    AFX_ASSERT(devices);
