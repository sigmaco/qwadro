...
_AVX afxUnit AvxDescribeVertexLayout(avxVertexInput vin, avxVertexDescription* layout)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_VIN, 1, &vin);
    AFX_ASSERT(layout);
...
afxError generateVertexBinAndAttr
(
    // Input streams
    avxVertexStream const* streams,
    // Number of streams
    afxSize streamCount,
    // Input attributes
    avxVertexAttr const* attrs,
...
{
    afxError err = { 0 };
    afxSize attrIdx = 0;  // Attribute index counter
    afxSize binIdx = 0;    // Bin index counter

    afxUnit usedPinCnt = 0;
    afxUnit usedPins[AVX_MAX_VERTEX_SOURCES];   // To track used 'pin' values
    afxUnit usedLocationsCnt = 0;
    afxUnit usedLocations[AVX_MAX_VERTEX_ATTRIBS]; // To track used 'location' values

    // Loop through all the streams
    for (afxSize i = 0; i < streamCount; ++i)
    {
        avxVertexStream const* stream = &streams[i];

        // Ensure 'pin' is unique
        for (afxSize j = 0; j < usedPinCnt; ++j)
        {
            if (usedPins[j] == stream->pin)
            {
                //throw std::runtime_error("Duplicate pin detected in stream.");
                AfxReportError("Duplicate pin detected in stream.");
                AfxThrowError();
            }
        }
        usedPins[usedPinCnt] = stream->pin;
        ++usedPinCnt;
        ...
        // Set the base attribute index and initialize the attribute count
        bin->baseAttrIdx = attrIdx;
        bin->attrCnt = 0;  // We'll update this later after counting attributes

        // Set flags based on stream's attributes (can be customized further)
        bin->flags = stream->flags;

        // Count how many attributes belong to this bin
        afxSize startAttrIdx = attrIdx;  // Start from the current attribute index
        while (attrIdx < attrCount && attrs[attrIdx].pin == stream->pin)
        {
            ++attrIdx;
        }
        bin->attrCnt = attrIdx - startAttrIdx;  // Number of attributes for this bin

        ++binIdx;  // Move to the next bin
    }

    // Now loop through all attributes and fill them in
    afxSize outputAttrIdx = 0;
    for (afxSize i = 0; i < attrCount; ++i)
    {
        avxVertexAttr const* attr = &attrs[i];

        // Ensure 'location' is unique
        for (afxSize j = 0; j < usedLocationsCnt; ++j)
        {
            if (usedLocations[j] == attr->location)
            {
                //throw std::runtime_error("Duplicate location detected in attribute.");
                AfxReportError("Duplicate location (%u) detected in attribute (%u). %p", attr->location, i, /*vin*/0);
                AfxThrowError();
            }
        }
        ...
        outputAttr->offset = attr->offset;
        outputAttr->flags = attr->flags;
...
_AVX afxError _AvxVtxdCtorCb(avxVertexInput vin, void** args, afxUnit invokeNo)
{
    ...
    {
        avxVertexStream2 const* fetch = &layout->bins[i];
...
#endif
    afxUnit totalAttrCnt = layout->attrCnt;
#if 0
    ...
    generateVertexBinAndAttr(layout->bins, layout->binCnt, layout->attrs, layout->attrCnt, vin->bins, vin->attrs);
