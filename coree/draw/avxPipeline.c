...
_AVX afxError _AvxParseXmlPipelineBlueprint(afxXmlNode const* xmle, afxUnit specId, avxPipelineConfig* pipb, avxShaderType shaderStages[], afxUri shaderUris[], afxString shaderFns[])
{
    ...
    AFX_ITERATE_CHAIN_B2F(child, parent, &xmle->children)
    {
        if (AfxCompareStrings(&child->name, 0, TRUE, 1, &AFX_STRING("Shader"), NIL))
        {
            ...
            if (shaderStages)
                shaderStages[config.progCnt] = shdStage;

            if (shaderUris)
                shaderUris[config.progCnt] = shdUri;

            if (shaderFns)
                shaderFns[config.progCnt] = shdFn;
...
_AVX afxBool AvxGetPipelineCodebase(avxPipeline pip, avxShader* codebase)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_PIP, 1, &pip);
...
_AVX afxBool AvxGetPipelineShader(avxPipeline pip, avxShaderType stage, afxUnit* progId, afxString* func)
{
    ...
    for (afxUnit slotIdx = 0; slotIdx < pip->progCnt; slotIdx++)
    {
        _avxProgram* pprog = &pip->progs[slotIdx];

        if (pprog->stage != stage)
            continue;

        AFX_ASSERT(progId || func);
        if (progId) *progId = pprog->progId;
        if (func) *func = pprog->fn.s;
...
_AVX afxUnit AvxGetPipelinePrograms(avxPipeline pip, afxIndex first, afxUnit cnt, afxUnit progs[], afxString funcs[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_PIP, 1, &pip);
    AFX_ASSERT_RANGE(pip->progCnt, first, cnt);
    AFX_ASSERT(progs || funcs);
    afxUnit hitCnt = 0;
    cnt = AFX_MIN(pip->progCnt, cnt);

    for (afxUnit i = 0; i < cnt; i++)
    {
        if (progs) progs[i] = pip->progs[i].progId;
        if (funcs) funcs[i] = pip->progs[i].fn.s;
...
_AVX afxError _AvxPipRelinkVertex(avxPipeline pip, avxVertexInput vin, afxBool gen)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_PIP, 1, &pip);

    avxVertexInput curr = pip->vin;

    while ((curr != vin) || gen)
    {
        if (curr)
        {
            AFX_ASSERT_OBJECTS(afxFcc_VIN, 1, &curr);
        ...
        if (!vin)
        {
            if (gen)
            {
                ...
                avxVertexLayout vtxi = { 0 };
                if (AvxAcquireVertexInputs(dsys, 1, &vtxi, &vin))
                {
                    AfxThrowError();
                    break; // while
                }
...
_AVX afxError _AvxPipRelinkLigature(avxPipeline pip, avxLigature liga, afxBool gen, afxBool user)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_PIP, 1, &pip);

    avxLigature curr = pip->liga;

    while ((curr != liga) || gen || (pip->isUserLiga != user))
    {
        if (curr)
        {
            AFX_ASSERT_OBJECTS(afxFcc_LIGA, 1, &curr);
        ...
        if (!liga)
        {
            if (gen)
            {
                ...
                if (AvxAcquireLigatures(dsys, 1, &ligc, &liga))
                {
                    AfxThrowError();
                    break; // while
                }
...
_AVX afxError _AvxReprogramPipeline(avxPipeline pip, afxUnit cnt, avxShaderSpecialization const specs[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_PIP, 1, &pip);
    AFX_ASSERT(specs);
    ...
    for (afxUnit opIt = 0; opIt < cnt; opIt++)
    {
        ...
        for (slotIdx = 0; slotIdx < pip->progCnt; slotIdx++)
        {
            pprog = &pip->progs[slotIdx];

            if (pprog->stage == spec->stage)
            {
                found = TRUE;
                break;
            }
        ...
        if (!found)
        {
            for (slotIdx = 0; slotIdx < pip->progCnt; slotIdx++)
            {
                ...
                if (pprog->stage == NIL) // if is unused
                {
                    found = TRUE;
                    pprog->stage = spec->stage;
                    break;
                }
            }
        }
        ...
        pprog = &pip->progs[slotIdx];
        pprog->progId = progId;
        AfxMakeString8(&pprog->fn, &spec->fn);
        ...
        if (!pip->isUserLiga)
        {
            ...
            for (afxUnit j = 0; j < pip->progCnt; j++)
                listedProgs[j] = pip->progs[j].progId;

            listedProgCnt = pip->progCnt;
            ...
            {
                if (_AvxPipRelinkLigature(pip, liga, FALSE, FALSE))
                    AfxThrowError();

                AfxDisposeObjects(1, &liga);
            }
        }
    }
    return err;
...
_AVX afxError _AvxPipDtorCb(avxPipeline pip)
{
    ...
    for (afxUnit i = pip->progCnt; i-- > 0;)
    {
        _avxProgram* slot = &pip->progs[i];
    ...
    AFX_TRY_ASSERT_OBJECTS(afxFcc_LIGA, 1, &pip->liga);
    if (_AvxPipRelinkLigature(pip, NIL, FALSE, FALSE))
        AfxThrowError();

    AFX_TRY_ASSERT_OBJECTS(afxFcc_VIN, 1, &pip->vin);
...
_AVX afxError _AvxPipCtorCb(avxPipeline pip, void** args, afxUnit invokeNo)
{
    ...
    afxUnit progCnt = 0;// pipb->progCnt;
    //AFX_ASSERT(progCnt);
    ...
    pip->progCnt = progCnt;
    for (afxUnit i = 0; i < pip->progCnt; i++)
    {
        _avxProgram* prog = &pip->progs[i];
        AfxZero(prog, sizeof(*prog));
    }

    if (pip->transformationDisabled) pip->progs[0].stage = avxShaderType_COMPUTE;
    else
    {
        afxUnit nextStageIdx = 0;
        pip->progs[nextStageIdx++].stage = avxShaderType_VERTEX;

        if (pip->tesselationEnabled)
        {
            pip->progs[nextStageIdx++].stage = avxShaderType_TESS_CTRL;
            pip->progs[nextStageIdx++].stage = avxShaderType_TESS_EVAL;
        }

        if (pipb->primShaderSupported)
        {
            pip->progs[nextStageIdx++].stage = avxShaderType_PRIMITIVE;
        }

        if (!pip->rasterizationDisabled)
        {
            pip->progs[nextStageIdx++].stage = avxShaderType_FRAGMENT;
        }
    }
    ...
    if (!err)
    {
        ...
        AFX_TRY_ASSERT_OBJECTS(afxFcc_VIN, 1, &vin);
        if (_AvxPipRelinkVertex(pip, vin, FALSE))
        {
            AfxThrowError();
        }

        pip->liga = NIL;
        pip->isUserLiga = FALSE;

        avxLigature liga = pipb->liga;
        AFX_TRY_ASSERT_OBJECTS(afxFcc_LIGA, 1, &liga);
        if (_AvxPipRelinkLigature(pip, liga, FALSE, liga ? TRUE : FALSE))
        {
            AfxThrowError();
        }
        ...
        AFX_ASSERT((!pipb->progSpecs) || (0 != pipb->progCnt));

        if (!err && pipb->progCnt && pipb->progSpecs)
        {
            if (_AvxReprogramPipeline(pip, pipb->progCnt, pipb->progSpecs))
            {
                AfxThrowError();
            }
        }
        ...
        if (err)
        {
            _AvxPipRelinkVertex(pip, NIL, FALSE);
            _AvxPipRelinkLigature(pip, NIL, FALSE, FALSE);
            _AvxPipRelinkShader(pip, NIL, FALSE);
        }
    }
...
_AVX afxError AvxAssembleGfxPipelines(afxDrawSystem dsys, afxUnit cnt, avxPipelineConfig const cfg[], avxPipeline pipelines[])
{
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        ...
        AFX_ASSERT(pip->progCnt >= c->progCnt);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                for (afxUnit i = 0; i < info.progCnt; i++)
                {
                    afxUri file;
