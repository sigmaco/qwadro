...
_ACX afxError AcxDumpBuffer(acxBuffer buf, afxUnit opCnt, acxBufferIo const ops[], void* dst, afxMask exuMask)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(AcxGetBufferFlags(buf, acxBufferFlag_R));
    AFX_ASSERT(opCnt);
    AFX_ASSERT(ops);
    AFX_ASSERT(dst);

#if ACX_VALIDATION_ENABLED
    // Validate the buffer operations (only happens when validation is enabled)
    for (afxUnit i = 0; i < opCnt; i++)
    {
        acxBufferIo const* op = &ops[i];
        AFX_ASSERT_ALIGNMENT(op->srcOffset, ACX_SBUFFER_ALIGNMENT);
        AFX_ASSERT_RANGE(AcxGetBufferCapacity(buf, 0), op->srcOffset, AFX_MAX(1, op->rowCnt) * AFX_MAX(1, op->dstStride));
    }
#endif//ACX_VALIDATION_ENABLED

    // Prepare the transfer structure for the dump operation
    acxTransference transfer = { 0 };
    ...
    afxWarpSystem ssys = AcxGetBufferHost(buf);
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);

    // Perform the transfer (dump) of data
    if (_AcxSsysGetDdi(ssys)->transferCb(ssys, &transfer, opCnt, ops))
    {
    ...
    if (AcxWaitForWarpQueue(ssys, AFX_TIMEOUT_INFINITE, portIdx, transfer.baseQueIdx))
        AfxThrowError();
#endif
...
_ACX afxError AcxUpdateBuffer(acxBuffer buf, afxUnit opCnt, acxBufferIo const ops[], void const* src, afxMask exuMask)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(AcxGetBufferFlags(buf, acxBufferFlag_W));
    AFX_ASSERT(opCnt);
    AFX_ASSERT(ops);
    AFX_ASSERT(src);
    ...
    {
        acxBufferIo const* op = &ops[i];
        AFX_ASSERT_ALIGNMENT(op->dstOffset, ACX_SBUFFER_ALIGNMENT);
        AFX_ASSERT_RANGE(AcxGetBufferCapacity(buf, 0), op->dstOffset, AFX_MAX(1, op->rowCnt) * AFX_MAX(1, op->dstStride));
    }
#endif//ACX_VALIDATION_ENABLED
...
_ACX afxError AcxUploadBuffer(acxBuffer buf, afxUnit opCnt, acxBufferIo const ops[], afxStream in, afxMask exuMask)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(AcxGetBufferFlags(buf, acxBufferFlag_W));
...
_ACX afxError AcxDownloadBuffer(acxBuffer buf, afxUnit opCnt, acxBufferIo const ops[], afxStream out, afxMask exuMask)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(AcxGetBufferFlags(buf, acxBufferFlag_R));
...
_ACX afxError AcxLoadBuffers(afxWarpSystem ssys, afxUnit cnt, afxUri const uris[], acxBuffer buffers[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    AFX_ASSERT(buffers);
    ...
    for (afxUnit idx = 0; idx < cnt; idx++)
    {
        ...
        acxBufferInfo bufi = { 0 };
        bufi.flags = acxBufferFlag_RWXC;
        bufi.usage = acxBufferUsage_UPLOAD;
        bufi.size = len;
        if (AcxAcquireBuffers(ssys, 1, &bufi, &buffers[idx]))
        {
            AfxThrowError();
            AfxDisposeObjects(idx, buffers);
            break;
...
_ACX afxError AcxMapBuffers(afxWarpSystem ssys, afxUnit cnt, acxBufferedMap maps[], void** placeholders[])
{
    afxError err = { 0 };
    ...
    {
        acxBufferedMap const* map = &maps[i];
        acxBuffer buf = map->buf;
        ...
        afxSize bufCap = AcxGetBufferCapacity(buf, 0);
        ...
        AFX_ASSERT_ALIGNMENT(offset, ACX_BUFFER_ALIGNMENT);
        // offset must be aligned to ACX_BUFFER_ALIGNMENT.
        ...
        afxWarpSystem ssys2 = AcxGetBufferHost(buf);
        if (ssys2 != ssys)
        {
        ...
        {
#ifndef _ACX_BUFFER_HOSTSIDE_ALWAYS_FULLY_MAPPED
    ...
    if (_AcxSsysGetDdi(ssys)->remapCb(ssys, FALSE, opCnt, remaps2))
    {
        AfxThrowError();
    }
...
_ACX afxError AcxUnmapBuffers(afxWarpSystem ssys, afxUnit cnt, acxBufferedMap maps[])
{
    afxError err = { 0 };
    // @ssys must be a valid afxWarpSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    ...
    */
    ...
    if (_AcxSsysGetDdi(ssys)->remapCb(ssys, TRUE, cnt, unmaps2))
    {
        AfxThrowError();
    }
...
_ACX afxError AcxCohereMappedBuffers(afxWarpSystem ssys, afxBool discard, afxUnit cnt, acxBufferedMap const maps[])
{
    afxError err = { 0 };
    // @ssys must be a valid afxWarpSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    ...
    if (_AcxSsysGetDdi(ssys)->cohereCb(ssys, discard, cnt, maps))
    {
...
_ACX afxError AcxCopyBuffers(afxWarpSystem ssys, afxMask exuMask, afxUnit cnt, acxBufferedCopy const ops[])
{
    afxError err = { 0 };
    // @ssys must be a valid afxWarpSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        acxBufferedCopy const* op = &ops[i];
        AFX_ASSERT_ALIGNMENT(op->dstOffset, ACX_BUFFER_ALIGNMENT);
        AFX_ASSERT_ALIGNMENT(op->srcOffset, ACX_BUFFER_ALIGNMENT);

        acxBuffer dst = op->dst;
        AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &dst);
        AFX_ASSERT_RANGE(AcxGetBufferCapacity(dst, 0), op->dstOffset, op->range);

        acxBuffer src = op->src;
        AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &src);
        AFX_ASSERT_RANGE(AcxGetBufferCapacity(src, 0), op->srcOffset, op->range);
        ...
        // Perform the data transfer (buffer update)
        if (_AcxSsysGetDdi(ssys)->transferCb(ssys, &transfer, 1, &op2))
        {
            // If transfer fails, throw an error
            AfxThrowError();
            //return err;
