...
_ACX afxError _AcxFillBuffer(acxBuffer buf, afxSize offset, afxSize range, afxUnit stride, afxUnit sampleCnt, afxUnit sampleRate, acxFormat fmt, afxReal amplitude, afxReal freq, afxReal dur)
{
    afxError err = { 0 };
...
_ACX afxError _AcxCopyBuffer(acxBuffer buf, acxBufferCopy const* op, acxBuffer dst)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(dst);
...
_ACX afxError _AcxDumpBuffer(acxBuffer buf, acxBufferIo const* op, void* dst)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(dst);
...
_ACX afxError _AcxUpdateBuffer(acxBuffer buf, acxBufferIo const* op, void const* src)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(src);
...
_ACX afxError _AcxDownloadBuffer(acxBuffer buf, acxBufferIo const* op, afxStream iob)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(iob);
...
_ACX afxError _AcxUploadBuffer(acxBuffer buf, acxBufferIo const* op, afxStream iob)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(iob);
