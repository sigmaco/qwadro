...
{
    afxError(*prepare)(afxStepContext, afxBool purge, acxCmdFlags);
...
#ifdef _ACX_STEP_CONTEXT_C
#ifdef _ACX_STEP_CONTEXT_IMPL
...
ACX afxError _AcxSctxPushCmd(afxStepContext sctx, afxUnit id, afxUnit siz, afxCmdId* cmdId, _acxCmd** ppCmd);
ACX _acxCmdBatch* _AcxSctxGetCmdBatch(afxStepContext sctx, afxUnit idx);
ACX afxError _AcxStepResetCb(afxStepContext sctx, afxBool freeMem, afxBool permanent);
ACX afxError _AcxStepEndCb(afxStepContext sctx);

ACX afxClassConfig const _ACX_SCTX_CLASS_CONFIG;

ACX _acxDdiSctx const _ACX_SCTX_DDI;

ACX acxContextState _AcxSctxGetStatus(afxStepContext sctx);
