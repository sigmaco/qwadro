...
_ACX afxError AcxDumpBuffer(acxBuffer buf, afxUnit opCnt, acxBufferIo const ops[], void* dst, afxMask exuMask)
{
    ...
    if (AcxWaitForStepQueue(ssys, AFX_TIMEOUT_INFINITE, portIdx, transfer.baseQueIdx))
        AfxThrowError();
#endif
...
_ACX afxError AcxLoadBuffers(afxStepSystem ssys, afxUnit cnt, afxUri const uris[], acxBuffer buffers[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    AFX_ASSERT(buffers);
...
_ACX afxError AcxMapBuffers(afxStepSystem ssys, afxUnit cnt, acxBufferedMap maps[], void** placeholders[])
{
    afxError err = { 0 };
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        ...
        afxStepSystem ssys2 = AcxGetBufferHost(buf);
        if (ssys2 != ssys)
        {
...
_ACX afxError AcxUnmapBuffers(afxStepSystem ssys, afxUnit cnt, acxBufferedMap maps[])
{
    afxError err = { 0 };
    // @ssys must be a valid afxStepSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
...
_ACX afxError AcxCohereMappedBuffers(afxStepSystem ssys, afxBool discard, afxUnit cnt, acxBufferedMap const maps[])
{
    afxError err = { 0 };
    // @ssys must be a valid afxStepSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
...
_ACX afxError AcxCopyBuffers(afxStepSystem ssys, afxMask exuMask, afxUnit cnt, acxBufferedCopy const ops[])
{
    afxError err = { 0 };
    // @ssys must be a valid afxStepSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
