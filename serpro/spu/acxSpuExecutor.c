...
_ACX afxError _AcxSpuWork_ExecuteCb(acxSpu* spu, _acxIoReqPacket* work)
{
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        ...
        _AcxSpuRollStepContexts(spu, sctx);

        // Must be disposed because AcxSubmitStepCommands() reacquires it.
...
_ACX afxBool _AcxSpu_ProcCb(acxSpu* spu)
{
    ...
    {
        afxStepQueue sque;
        if (!AcxGetStepQueues(sexu, queIdx, 1, &sque) || !sque)
        {
            AfxThrowError();
            continue;
        }
...
_ACX afxInt _ACX_SPU_THREAD_PROC(afxStepBridge sexu)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
    ...
    afxUnit exuId;
    afxStepSystem ssys = AcxGetBridgedStepSystem(sexu, &exuId);
...
_ACX afxError _AcxSexu_PingCb(afxStepBridge sexu, afxUnit queIdx)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
    AfxLockMutex(&sexu->schedCndMtx);
