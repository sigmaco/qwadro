...
_ASX afxError AsxDumpBuffer(asxBuffer buf, afxUnit opCnt, asxBufferIo const ops[], void* dst, afxMask exuMask)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(AsxGetBufferFlags(buf, asxBufferFlag_R));
    AFX_ASSERT(opCnt);
    AFX_ASSERT(ops);
    AFX_ASSERT(dst);

#if ASX_VALIDATION_ENABLED
    // Validate the buffer operations (only happens when validation is enabled)
    for (afxUnit i = 0; i < opCnt; i++)
    {
        asxBufferIo const* op = &ops[i];
        AFX_ASSERT_ALIGNMENT(op->srcOffset, ASX_SBUFFER_ALIGNMENT);
        AFX_ASSERT_RANGE(AsxGetBufferCapacity(buf, 0), op->srcOffset, AFX_MAX(1, op->rowCnt) * AFX_MAX(1, op->dstStride));
    }
#endif//ASX_VALIDATION_ENABLED

    // Prepare the transfer structure for the dump operation
    asxTransference transfer = { 0 };
    transfer.exuMask = exuMask;
    transfer.srcFcc = afxFcc_SBUF;
    ...
    if (_AsxSsysGetDdi(ssys)->transferCb(ssys, &transfer, opCnt, ops))
    {
    ...
    if (AsxWaitForStepQueue(ssys, AFX_TIMEOUT_INFINITE, portIdx, transfer.baseQueIdx))
        AfxThrowError();
#endif
...
_ASX afxError AsxUpdateBuffer(asxBuffer buf, afxUnit opCnt, asxBufferIo const ops[], void const* src, afxMask exuMask)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(AsxGetBufferFlags(buf, asxBufferFlag_W));
    AFX_ASSERT(opCnt);
    AFX_ASSERT(ops);
    AFX_ASSERT(src);
    ...
    {
        asxBufferIo const* op = &ops[i];
        AFX_ASSERT_ALIGNMENT(op->dstOffset, ASX_SBUFFER_ALIGNMENT);
        AFX_ASSERT_RANGE(AsxGetBufferCapacity(buf, 0), op->dstOffset, AFX_MAX(1, op->rowCnt) * AFX_MAX(1, op->dstStride));
    }
#endif//ASX_VALIDATION_ENABLED
...
_ASX afxError AsxUploadBuffer(asxBuffer buf, afxUnit opCnt, asxBufferIo const ops[], afxStream in, afxMask exuMask)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(AsxGetBufferFlags(buf, asxBufferFlag_W));
...
_ASX afxError AsxDownloadBuffer(asxBuffer buf, afxUnit opCnt, asxBufferIo const ops[], afxStream out, afxMask exuMask)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &buf);
    AFX_ASSERT(AsxGetBufferFlags(buf, asxBufferFlag_R));
...
_ASX afxError AsxLoadBuffers(afxStepSystem ssys, afxUnit cnt, afxUri const uris[], asxBuffer buffers[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    AFX_ASSERT(buffers);
    ...
    for (afxUnit idx = 0; idx < cnt; idx++)
    {
        ...
        bufi.flags = asxBufferFlag_RWXC;
        bufi.usage = asxBufferUsage_UPLOAD;
        bufi.size = len;
        if (AsxAcquireBuffers(ssys, 1, &bufi, &buffers[idx]))
        {
            AfxThrowError();
            AfxDisposeObjects(idx, buffers);
            break;
...
_ASX afxError AsxMapBuffers(afxStepSystem ssys, afxUnit cnt, asxBufferedMap maps[], void** placeholders[])
{
    afxError err = { 0 };
    ...
    _asxBufferRemapping remaps2[32];
    // Count of operations to be processed
    afxUnit opCnt = 0;

    for (afxUnit i = 0; i < cnt; i++)
    {
        asxBufferedMap const* map = &maps[i];
        asxBuffer buf = map->buf;
        ...
        afxSize bufCap = AsxGetBufferCapacity(buf, 0);
        ...
        AFX_ASSERT_ALIGNMENT(offset, ASX_BUFFER_ALIGNMENT);
        // offset must be aligned to ASX_BUFFER_ALIGNMENT.
        ...
        afxStepSystem ssys2 = AsxGetBufferHost(buf);
        if (ssys2 != ssys)
        {
            AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys2);
            AFX_ASSERT(ssys2 == ssys);
            AfxThrowError();
            break;
        }
        ...
        {
#ifndef _ASX_BUFFER_HOSTSIDE_ALWAYS_FULLY_MAPPED
    ...
    if (_AsxSsysGetDdi(ssys)->remapCb(ssys, FALSE, opCnt, remaps2))
    {
        AfxThrowError();
    }
...
_ASX afxError AsxUnmapBuffers(afxStepSystem ssys, afxUnit cnt, asxBufferedMap maps[])
{
    afxError err = { 0 };
    // @ssys must be a valid afxStepSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    ...
    */
    ...
    // Ensure that the operation count doesn't exceed 32
    AFX_ASSERT(cnt <= 32);
    _asxBufferRemapping unmaps2[32];
    ...
    if (_AsxSsysGetDdi(ssys)->remapCb(ssys, TRUE, cnt, unmaps2))
    {
        AfxThrowError();
    }
...
_ASX afxError AsxCohereMappedBuffers(afxStepSystem ssys, afxBool discard, afxUnit cnt, asxBufferedMap const maps[])
{
    afxError err = { 0 };
    // @ssys must be a valid afxStepSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    ...
    if (_AsxSsysGetDdi(ssys)->cohereCb(ssys, discard, cnt, maps))
    {
...
_ASX afxError AsxCopyBuffers(afxStepSystem ssys, afxMask exuMask, afxUnit cnt, asxBufferedCopy const ops[])
{
    afxError err = { 0 };
    // @ssys must be a valid afxStepSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        asxBufferedCopy const* op = &ops[i];
        AFX_ASSERT_ALIGNMENT(op->dstOffset, ASX_BUFFER_ALIGNMENT);
        AFX_ASSERT_ALIGNMENT(op->srcOffset, ASX_BUFFER_ALIGNMENT);

        asxBuffer dst = op->dst;
        AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &dst);
        AFX_ASSERT_RANGE(AsxGetBufferCapacity(dst, 0), op->dstOffset, op->range);

        asxBuffer src = op->src;
        AFX_ASSERT_OBJECTS(afxFcc_SBUF, 1, &src);
        AFX_ASSERT_RANGE(AsxGetBufferCapacity(src, 0), op->srcOffset, op->range);
        ...
        // Perform the data transfer (buffer update)
        if (_AsxSsysGetDdi(ssys)->transferCb(ssys, &transfer, 1, &op2))
        {
            // If transfer fails, throw an error
            AfxThrowError();
            //return err;
