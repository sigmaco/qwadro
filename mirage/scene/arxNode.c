...
_ARX afxBool ArxGetNode(arxNodular* nodu, arxNode* node)
{
    afxError err = { 0 };
    AFX_ASSERT(nodu);
    arxNode curr = AfxGetLinker(&nodu->nod);
    AFX_TRY_ASSERT_OBJECTS(afxFcc_NOD, 1, &curr);
    AFX_ASSERT(node);
    *node = curr;
    return !!curr;
}

_ARX afxError ArxBreakNodulation(arxNodular* nodu)
{
    afxError err = { 0 };
    AFX_ASSERT(nodu);
...
_ARX afxError ArxMakeNodulation(arxNodular* nodu, arxNode nod, void(*sync)(arxNodular*), afxFlags dagFlags, afxMask dagMask)
{
    afxError err = { 0 };
    AFX_ASSERT(nodu);
    ...
    if (curr != nod)
    {
        ...
        if (nod)
        {
            ...
            AfxPushLink(&nodu->nod, &nod->nodulars);
        }
    }
    ...
    if (curr != parent)
    {
        if (curr)
        {
            if (curr == AfxGetHost(nod))
            {
                ...
                AFX_ASSERT_OBJECTS(afxFcc_SCIO, 1, &curr);
                AfxPopLink(&nod->parent);
                AFX_ASSERT(nod->root == nod);
...
_ARX afxError ArxComputeDagMotionVectors(arxNode nod, afxReal secsElapsed, afxBool inv, afxV3d translation, afxV3d rotation)
{
    ...
    {
        ArxComputePuppetMotionVectors(nod->pup, secsElapsed, inv, translation, rotation);
        break;
    }
    ...
    case arxNodeType_Node_WeightedBlend:
    {
        ...
        arxNode it;
        AFX_ITERATE_CHAIN(it, parent, &n->children)
        {
            afxReal w = it->weight;
...
_ARX afxError ArxStepDag(arxNode nod, afxReal time)
{
    ...
    case arxNodeType_Leaf_AnimBlend:
    {
        ArxStepPuppetMotives(nod->pup, time);
        break;
    }
    case arxNodeType_Leaf_Callback:
    {
...
_ARX afxError ArxSampleDag(arxNode nod, afxReal allowedErr, arxPose pose, afxUnit jntCnt, afxUnit const sparseJntMap[], void* cache)
{
    ...
    arxScenario scio = AfxGetHost(nod);
    ...
    L26:
        ...
        ArxModulatePose(pose, 0, poseB, 0, ArxGetPoseCapacity(pose, 0), nod->weightNone, nod->weightAll, nod->trackMask, sparseJntMap);
        break;
    }
    case arxNodeType_Node_WeightedBlend:
    {
        ...
        arxNode child;
        AFX_ITERATE_CHAIN(child, parent, &n->children)
        {
...
_ARXINL afxError _AsxNodDtorCb(arxNode nod)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_NOD, 1, &nod);
...
_ARXINL afxError _AsxNodCtorCb(arxNode nod, void** args, afxUnit invokeNo)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_NOD, 1, &nod);
...
_ARX afxClassConfig const _ARX_NOD_CLASS_CONFIG =
{
    .fcc = afxFcc_NOD,
    .name = "Node",
    .desc = "Node",
...
_ARXINL afxError ArxAcquireNodes(arxScenario scio, afxUnit cnt, arxNode nod[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCIO, 1, &scio);

    if (AfxAcquireObjects((afxClass *)_ArxScioGetNodClass(scio), cnt, (afxObject*)nod, (void const*[]) { scio }))
        AfxThrowError();
...
_ARX afxError ArxAcquireJunctionNode(arxScenario scio, arxNode parent, arxModel skl, afxQuatBlend blendOp, afxReal fillThreshold, arxNode* node)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCIO, 1, &scio);

    arxNode nod;
    ArxAcquireNodes(scio, 1, &nod);
...
_ARX afxError ArxAcquireCrossfadeNode(arxScenario scio, arxNode parent, arxNode a, arxNode b, afxReal weightNone, afxReal weightAll, arxTrackMask* trackMask, arxNode* node)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCIO, 1, &scio);
...
_ARX afxError ArxAcquirePoseNode(arxScenario scio, arxNode parent, arxPose pose, arxNode* node)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCIO, 1, &scio);
...
_ARX afxError ArxAcquireAnimationNode(arxScenario scio, arxNode parent, arxPuppet pup, afxReal fillThreshold, arxNode* node)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCIO, 1, &scio);
...
_ARX afxError ArxAcquirePostureNode(arxScenario scio, arxNode parent, arxPosture plce, arxNode* node)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCIO, 1, &scio);
    ...
    {
        afxUnit artCnt = 256;
        //ArxAcquirePostures(scio, 1, &artCnt, NIL, &plce);
    }
    //nod->plce = plce;
    //nod->pose.owned = TRUE;
...
_ARX afxError ArxCaptureNodes(arxScenario scio, arxDagCaptureFlags flags, afxBool(*cb)(arxNode nod, void *udd), void *udd, afxArray* pvs)
{
    afxError err = { 0 };
    ...
    if (!cb)
        cb = (void*)_ArxCaptureNodCb;

    void** udd2 = (void*[]) { cb, &lodErr, pvs };

    arxNode root;
    afxChain const* dags = _ArxScioGetDagRoots(scio);
    AFX_ITERATE_CHAIN_B2F(root, parent, dags)
    {
        ...
        arxNode nod;
        AFX_ITERATE_CHAIN(nod, parent, &root->children)
        {
            AFX_ASSERT_OBJECTS(afxFcc_NOD, 1, &nod);
...
_ARX afxError ArxCaptureNodesInFrustum(arxScenario scio, arxDagCaptureFlags flags, afxBool(*cb)(arxNode nod, void *udd), void *udd, afxFrustum const* bounds, afxArray* pvs)
{
    ...
    AFX_ITERATE_CHAIN_B2F(root, parent, dags)
    {
        ...
        AFX_ITERATE_CHAIN(nod, parent, &root->children)
        {
            ...
            arxNode child;
            AFX_ITERATE_CHAIN_B(child, parent, &nod->children)
            {
                AFX_ASSERT_OBJECTS(afxFcc_NOD, 1, &child);
...
_ARX void _ArxNodUpdatePlacement(arxNode nod, afxReal allowedErr)
{
    ...
    if (nod->type == arxNodeType_Leaf_AnimBlend)
    {
        ...
        arxPose pose;
        arxPosture plce;
        ArxGetPuppetPose(pup, &pose, &plce);

        afxUnit jntCnt = ArxGetSkeletonDepth(skl);
        ...
        {
            ArxProducePosture(plce, NIL, skl, 0, jntCnt, 0, jntCnt, nod->w, FALSE);
        }
        else
        {
            ...
            ArxCommencePoseAccumulation(pose, 0, jntCnt, NIL);
            ArxAccumulatePuppetAnimations(pose, pup, 0, jntCnt, allowedErr, NIL);
            ArxConcludePoseAccumulation(pose, 0, jntCnt, mdl, allowedErr, NIL);
            ArxProducePosture(plce, pose, skl, 0, jntCnt, 0, lodJntCnt, nod->w, FALSE);
        }
    }
}

_ARX afxError ArxUpdateDags(afxReal allowedErr, arxDagUpdateFlags flags, afxUnit cnt, arxNode nodes[])
{
    afxError err;
    AFX_ASSERT(nodes);
    AFX_ASSERT(cnt);
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        ...
        arxNodular* nodu;
        AFX_ITERATE_CHAIN_B2F(nodu, nod, &nod->nodulars)
        {
            nodu->sync(nodu);
        };
    }
...
_ARX afxError ArxStepDags(afxReal time, arxDagStepFlags flags, afxUnit cnt, arxNode nodes[])
{
    afxError err;
    AFX_ASSERT(nodes);
    AFX_ASSERT(cnt);
