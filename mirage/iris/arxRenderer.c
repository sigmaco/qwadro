...
#if 0
_ARX afxBool _AsxCaptureBodCb(arxPuppet bod, void** udd)
{
    afxError err = { 0 };
...
_ARX afxError AsxCapturePuppetPvs(arxSimulation sim, arxCamera cam, afxReal lodErr, afxArray* pvs)
{
    afxError err = { 0 };
    ...
    ArxInvokePuppets(sim, 0, AFX_U32_MAX, (void*)_AsxCaptureBodCb, (void*[]) { &frustum, &lodErr, pvs });

    return err;
}
#endif

_ARX afxError ArxDrawBodies(arxRenderer scn, arxContext sctx, afxDrawContext dctx, afxUnit cnt, arxPuppet bodies[])
{
    afxError err = { 0 };
    ...
    {
        arxPuppet bod = bodies[bodIdx];
        AFX_ASSERT_OBJECTS(afxFcc_PUP, 1, &bod);
        arxModel mdl;

        if (!ArxGetPuppetModel(bod, &mdl))
            continue;
        ...
        //ArxComputePuppetMotionMatrix(bod, dt, FALSE, bod->placement, bod->placement);
        //AfxSamplePuppetAnimations(bod, 0, bod->cachedBoneCnt, scn->lp, 0.0, NIL);
        //ArxProducePosture(skl, 0, bod->cachedBoneCnt, scn->lp, m, scn->wp);
        //AfxComputeRestPosture(skl, 0, bod->cachedBoneCnt, m, scn->wp);

        arxPosture bodPlce;
        ArxGetPuppetPose(bod, NIL, &bodPlce);
#if !0
        AsxCmdSamplePuppetAnimationsAcceleratedLOD(sctx, bod, jntCnt, AFX_M4D_IDENTITY, scn->lp, scn->wp[scn->frameIdx], 0.0);
        //AfxSamplePuppetAnimationsAccelerated(bod, jntCnt, bod->placement, scn->lp, scn->wp, 0.0);
#else
        afxM4d mdlDispl;
        AfxComputeModelDisplacement(mdl, mdlDispl);
        AfxSamplePuppetAnimations(bod, 0, sampledJntCnt, bod->pose, sampleLodErr, NIL);
        ...
        {
            arxMeshRigInfo rig;
            ArxDescribeMeshRigs(mdl, mshIdx, 1, &rig);
...
#if 0
_ARX afxError ArxBeginSceneCapture(arxRenderer scn, arxCamera cam, arxSimulation sim, afxDrawContext dctx, arxContext sctx)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_RND, 1, &scn);
    AFX_ASSERT_OBJECTS(afxFcc_CAM, 1, &cam);
    ...
    AsxCapturePuppetPvs(sim, cam, 0, &scn->capturedNodes);
    ...
    arxPuppet bod;
    afxUnit i = 0;
    while (ArxEnumeratePuppets(sim, i++, 1, &bod))
    {
        AFX_ASSERT_OBJECTS(afxFcc_PUP, 1, &bod);
        arxModel mdl;
...
#if !0
            AmxCmdSamplePuppetAnimationsAcceleratedLOD(NIL, bod, jntCnt, bod->placement, scn->lp, scn->wp, 0.0);
#else
            afxM4d mdlDispl;
            AfxComputeModelDisplacement(mdl, mdlDispl);
            AfxSamplePuppetAnimations(bod, 0, sampledJntCnt, bod->pose, sampleLodErr, NIL);
...
_ARX afxError ArxCmdDrawBodies(afxDrawContext dctx, arxRenderer rnd, afxReal dt, afxUnit cnt, arxPuppet bodies[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_RND, 1, &rnd);
    AFX_ASSERT_OBJECTS(afxFcc_PUP, cnt, bodies);
    ...
    for (afxUnit bodIdx = 0; bodIdx < cnt; bodIdx++)
    {
        ...
        //AfxGetThreadTime(&ct, &dt);
        //ArxComputePuppetMotionMatrix(bod, dt, FALSE, bod->placement, bod->placement);
        //AmxCmdSamplePuppetAnimationsAcceleratedLOD(sctx, bod, ArxCountJoints(mdl, 0), bod->placement, rnd->lp, rnd->wp, 0.0);
        AsxCmdSamplePuppetAnimationsAcceleratedLOD(NIL, bod, ArxCountJoints(mdl, 0), AFX_M4D_IDENTITY, rnd->lp, rnd->wp[rnd->frameIdx], 0.0);
        //AfxSamplePuppetAnimations(bod, 0, bod->cachedBoneCnt, rnd->lp, 0.0, NIL);
        //ArxProducePosture(skl, 0, bod->cachedBoneCnt, rnd->lp, m, rnd->wp);
...
_ARX afxError AkxTestDrawIndexed(arxRenderer rnd, afxDrawContext dctx)
{
    AvxCmdUseDrawTechniqueSIGMA(dctx, rnd->testDtec, 0, rnd->testVin, NIL);
...
_ARX afxError ArxRendererSetStar(arxRenderer rnd, afxV4d const pos, afxV3d const dir, afxV4d const Kd)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_RND, 1, &rnd);
...
_ARX afxError ArxEndSceneRendering(arxRenderer rnd, afxDrawContext dctx)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_RND, 1, &rnd);
...
_ARX afxError ArxBeginSceneRendering(arxRenderer rnd, arxCamera cam, afxLayeredRect const* drawArea, avxCanvas canv, afxDrawContext dctx)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_RND, 1, &rnd);
    ...
    {
        AvxGetCanvasExtent(canv, NIL, &rnd->drawArea);

    }
    else
    {
        rnd->drawArea = (afxLayeredRect) { 0 };
    }

    arxViewConstants *viewConstants = &rnd->framesets[frameIdx].viewConstants;
    ...
    AvxCmdBindBuffers(dctx, avxBus_GFX, 0, 0, 1, (avxBufferedMap[]) { {.buf = rnd->framesets[frameIdx].viewUbo } });
    return err;
}

_ARX afxError _ArxRndDtor(arxRenderer rnd)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_RND, 1, &rnd);
    ...
    arxScenario scio = rnd->scio;

    AfxDisposeObjects(1, &scio);
#if 0
    for (afxUnit i = 0; i < rnd->frameCnt; i++)
    {
        if (rnd->canv[i])
...
_ARX afxError _ArxRndCtor(arxRenderer rnd, void** args, afxUnit invokeNo)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_RND, 1, &rnd);
    ...
    if (!config->scio)
    {
        ...
        ArxAcquireScenario(0, &dinConfig, &scio);
        AFX_ASSERT(scio);
    }
    else
    {
        AfxReacquireObjects(1, &scio);
    }
    rnd->scio = scio;
...

    {
        ...
        AvxUpdateBuffer(rnd->testVbo, 1, &vboIop, testVertices, 0, NIL);
        ...
        AvxUpdateBuffer(rnd->testIbo, 1, &iboIop, testIndices, 0, NIL);
        ...
        ArxLoadRenderTechnique(scio, &uri, &rnd->testDtec);
    }
    ...
    AvxAcquireVertexInputs(dsys, 1, &skinVtxl, &skinnedVin);
    ...
    AvxAcquireVertexInputs(dsys, 1, &rigidVtxl, &rigidVin);
...

    {
        ...
        AvxAcquireVertexInputs(dsys, 1, &vtxl_p3n3t2_bi_mtl_mtx_jnt, &vin_p3n3t2_bi_mtl_mtx_jnt);
        ...
        AvxAcquireVertexInputs(dsys, 1, &vtxl_p3j1n3t2_bi_mtl_mtx_jnt, &vin_p3j1n3t2_bi_mtl_mtx_jnt);
        ...
        AvxAcquireVertexInputs(dsys, 1, &vtxl_p3j2n3t2_bi_mtl_mtx_jnt, &vin_p3j2n3t2_bi_mtl_mtx_jnt);
        ...
        AvxAcquireVertexInputs(dsys, 1, &vtxl_p3j3n3t2_bi_mtl_mtx_jnt, &vin_p3j3n3t2_bi_mtl_mtx_jnt);
        ...
        AvxAcquireVertexInputs(dsys, 1, &vtxl_p3j4n3t2_bi_mtl_mtx_jnt, &vin_p3j4n3t2_bi_mtl_mtx_jnt);
    ...
    }
    {
        ...
        {
            { .size = sizeof(arxViewConstants), .flags = avxBufferFlag_WX | avxBufferFlag_C, .usage = avxBufferUsage_UNIFORM },
            { .size = sizeof(arxShaderConstants), .flags = avxBufferFlag_WX | avxBufferFlag_C, .usage = avxBufferUsage_UNIFORM },
            { .size = sizeof(arxMaterialConstants), .flags = avxBufferFlag_WX | avxBufferFlag_C, .usage = avxBufferUsage_UNIFORM },
        ...
        for (afxUnit i = 0; i < rnd->frameCnt; i++)
        {
            ...
            if (AvxMapBuffers(dsys, ARRAY_SIZE(maps), maps, (void**[]) { (void**)&mtboPtr, (void**)&bmboPtr, (void**)&mtlboPtr, (void**)&icboPtr }))
                AfxThrowError();
...

    {
        ...
        ArxLoadRenderTechnique(scio, &uri, &rnd->bodyDtec);
    }
    ...
    arxPoseInfo posei = { 0 };
    posei.artCnt = 255;
    ArxAcquirePoses(rnd->scio, 1, &posei, &rnd->lp);
    ArxAcquirePostures(rnd->scio, 1, (afxUnit[]) { 255 }, (afxBool[]) {TRUE}, &rnd->wp[0]);
    ArxAcquirePostures(rnd->scio, 1, (afxUnit[]) { 255 }, (afxBool[]) { TRUE }, &rnd->wp[1]);

    AfxMakeUri(&uri, 0, "../gfx/testLighting/testLighting.xsh.xml", 0);
    ArxLoadRenderTechnique(scio, &uri, &rnd->blinnTestRazrDtec);

    AfxMakeUri(&uri, 0, "../gfx/tutCamUtil/tutCamUtil.xsh.xml", 0);
    ArxLoadRenderTechnique(scio, &uri, &rnd->tutCamUtilDtec);

    AfxMakeUri(&uri, 0, "../gfx/lighting/lighting.xsh.xml", 0);
    ArxLoadRenderTechnique(scio, &uri, &rnd->lightingDtec);

    AfxMakeArray(&rnd->capturedNodes, sizeof(arxPuppet), 100, NIL, 0);
...
_ARX afxClassConfig const _ARX_RND_CLASS_CONFIG =
{
    .fcc = afxFcc_RND,
    .name = "Renderer",
    .fixedSiz = sizeof(AFX_OBJECT(arxRenderer)),
...
_ARX afxError ArxAcquireRenderers(arxScenario scio, afxUnit cnt, arxRenderer rnd[], akxRendererConfig const config[])
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SCIO, 1, &scio);

    if (AfxAcquireObjects((afxClass *)_ArxScioGetRndrClass(scio), cnt, (afxObject*)rnd, (void const*[]) { scio, (void*)config }))
        AfxThrowError();
