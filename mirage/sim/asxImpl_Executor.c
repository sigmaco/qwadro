...
_ASX afxError _AsxSpuWork_ExecuteCb(asxSpu* spu, asxWork* work)
{
    ...
    {
        arxContext sctx = work->Execute.cmdbs[i];
...
_ASX afxBool _AsxSpu_ProcCb(asxSpu* spu)
{
    ...
    {
        arxSimQueue sque;
        if (!ArxGetSimQueues(sexu, queIdx, 1, &sque) || !sque)
        {
            AfxThrowError();
            continue;
        }
        ...
        {
            asxWork* work;
            AFX_ITERATE_CHAIN_B2F(work, hdr.chain, &sque->workChn)
            {
...
_ASX afxInt _ASX_SPU_THREAD_PROC(arxSimBridge sexu)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
...
_ASX afxError _AsxSexu_PingCb(arxSimBridge sexu, afxUnit queIdx)
{
    afxError err = { 0 };
    AFX_ASSERT_OBJECTS(afxFcc_SEXU, 1, &sexu);
    sexu->schedCnt += 1;
