...
(
    ...
    // If you want to start enumerating from the first available stepping system, you would set this value to 0. 
    ...
    // An array where the enumerated stepping systems will be stored. 
    // After the function call, the array will contain the stepping systems up to the requested count @cnt.
    afxStepSystem systems[]
);
...
(
    ...
    // The starting index for the enumeration of stepping systems. 
    ...
    // A callback function that will be invoked for each stepping system being enumerated.
    afxBool(*f)(void*, afxStepSystem),

    // The number of stepping systems to process starting from the first index. 
    // The function will invoke the callback for each of these stepping systems.
    afxUnit cnt
);
...
(
    ...
    // The index of the first stepping system to retrieve. 
    // This allows you to start processing from a specific stepping system rather than always starting from the first.
    afxUnit first,

    // The user-defined data that will be passed to the callback function.
    void* udd,

    // The callback function that will be invoked for each stepping system.
    afxBool(*f)(void*, afxStepSystem),
    ...
    // An array where the retrieved stepping systems will be stored. 
    // The function will fill this array with the actual stepping systems that were retrieved, up to the number @cnt.
    afxStepSystem systems[]
);
...
(
    ...
    // An array where the function will store the retrieved stepping bridges.
...
(
    ...
    // An array that will hold the selected stepping bridges.
    afxStepBridge bridges[]
);
...
(
    ...
    // It is expressed in microseconds and defines how long to wait for the stepping system 
    // to be ready or for the stepping operation to complete.
    afxTime timeout
);
...
(
    ...
    // If the stepping system has multiple bridges or execution units (e.g., for communication between different components or devices), 
...
(
    ...
    // The execution unit index, which likely refers to a specific queue or processing unit within the stepping system.
